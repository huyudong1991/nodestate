...
SSR_folder="/usr/local/shadowsocks"
...
Continue_if(){
	...
	if [[ ${Like_num} == ${integer} ]]; then
		Config_JSON="${Config_JSON}{ \"ip\": \"${ip}\", \"name\": \"${Config_Name}\", \"type\": \"${Config_Type}\", \"type_1\": \"${Config_Type_1}\", \"location\": \"${Config_Location}\", \"status\": ${Config_Status}, \"time\": \"$(date '+%Y-%m-%d %H:%M:%S')\",\"PING_DELAY\": \"${PING_DELAY}\"  }\n"
	else
		Config_JSON="${Config_JSON}{ \"ip\": \"${ip}\", \"name\": \"${Config_Name}\", \"type\": \"${Config_Type}\", \"type_1\": \"${Config_Type_1}\", \"location\": \"${Config_Location}\", \"status\": ${Config_Status}, \"time\": \"$(date '+%Y-%m-%d %H:%M:%S')\",\"PING_DELAY\": \"${PING_DELAY}\"  },\n"
	fi
	continue
...
Socks5_test(){
	...
	PING_TEST=$(ping ${ip} -c 3 > /root/tmp)
        PACKET_LOSS_RESULTS=$(cat /root/tmp | grep -a transmitted | awk '{print $6}')
        PING_DELAY=$(cat /root/tmp | grep -a rtt | cut -d "/" -f 6 | awk -F. '{print $1}')
	PING_DELAY=`echo $PING_DELAY|awk -F ' ' '{print $1}'`
	echo "ping值："${PING_DELAY}
...
Download_SSRStatus(){
	cd "/usr/local"
	wget -N --no-check-certificate "https://github.com/huyudong1991/nodestate/archive/master.zip"
	[[ ! -e "master.zip" ]] && echo -e "${Error} SSRStatus 网页文件下载失败 !" && exit 1
	unzip master.zip && rm -rf master.zip
	[[ ! -e "nodestate-master" ]] && echo -e "${Error} SSRStatus 网页文件解压失败 !" && exit 1
	mv nodestate-master SSRStatus
	[[ ! -e "${Web_file}" ]] && echo -e "${Error} SSRStatus 网页文件文件夹重命名失败 !" && rm -rf nodestate-master && exit 1
}
Install_Web(){
